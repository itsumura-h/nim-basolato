...
"""
  ...
  if isDirExists(&"{getCurrentDir()}/app/data_stores/query_services/{target}"): return 1
  if isDirExists(&"{getCurrentDir()}/app/usecases/{target}"): return 1

  var targetPath = &"{getCurrentDir()}/app/usecases/{target}/{targetName}_usecase.nim"
  ...
  targetPath = &"{getCurrentDir()}/app/usecases/{target}/{targetName}_query_service_interface.nim"
  if isFileExists(targetPath): return 1
  createDir(parentDir(targetPath))
  ...
  # query service
  targetPath = &"{getCurrentDir()}/app/data_stores/query_services/{target}/{targetName}_query_service.nim"
  if isFileExists(targetPath): return 1
  createDir(parentDir(targetPath))
  f = open(targetPath, fmWrite)
