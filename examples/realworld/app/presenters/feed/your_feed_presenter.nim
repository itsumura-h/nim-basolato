...
type YourFeedPresenter* = object
  articleListDao: IYourFeedArticleListDao
  articleCountDao: IYourFeedArticleCountDao

proc new*(_:type YourFeedPresenter):YourFeedPresenter =
  return YourFeedPresenter(
    articleListDao: di.yourFeedArticleListDao,
    articleCountDao: di.yourFeedArticleCountDao
  )


proc invoke*(self: YourFeedPresenter):Future[FeedTemplateModel] {.async.} =
  let context = context()
  let loginUserId = context.get("user_id").await
  let page = context.params.getInt("page", 1)
  let offset = (page - 1) * FEED_DISPLAY_COUNT

  let articleDtoList = self.articleListDao.invoke(loginUserId, offset,  FEED_DISPLAY_COUNT).await
  ...
  let totalCount = self.articleCountDao.invoke(loginUserId).await
